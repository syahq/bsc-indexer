...
import { PANCAKE_V2_HANDLER_ADDRESSES } from '../contracts'
...
export default class PancakeV2Relayer {
  base: Relayer
  pancakeV2Handler: Contract
  ...
  constructor(base: Relayer) {
    ...
    this.pancakeV2Handler = new Contract(
      PANCAKE_V2_HANDLER_ADDRESSES[base.chainId],
      HandlerABI,
      base.account
    )
  }
  ...
  async execute(order: Order): Promise<string | undefined> {
    // Get handler to use
    const handler = this.pancakeV2Handler
    if (!handler) {
      return
    }
