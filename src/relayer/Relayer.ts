...
import PancakeV2Relayer from './PancakeV2Relayer'
...
import unidexCoreBI from '../contracts/abis/UniDexCore.json'
...
export default class Relayer {
  ...
  pancakev2Relayer: PancakeV2Relayer
  ...
  constructor(provider: JsonRpcProvider) {
    ...
    this.pancakev2Relayer = new PancakeV2Relayer(this)
  ...
  async executeOrder(order: Order): Promise<string | undefined> {
    ...
    if (handler === 'balancer') {
      ...
      return this.pancakev2Relayer.execute(order)
    }
  }
  ...
  async estimateGasExecution(
    ...
    try {
      return await this.unidexCore.estimateGas.executeOrder(...params, {
        gasPrice
      })
    } catch (e) {
